...
# from config.config import get_gemini_client, get_llm_model, get_gpt_model, get_gpt_client
...
#     history = chat.get_recent_messages(chat_id, max_messages=3)
...
#     messages = [{"role": "system", "content": system_instruction_text}] + history
...
from core.chat_core import Chat
from core.farmer_core import Farmer
from core.helper_core import load_prompt, call_openai, extract_json, store_message_faq, get_max_messages, handle_log
...
def handle_health_log(chat_id, user_id, prompt):
    return handle_log(chat_id, user_id, prompt, "prompts/ask_farmer_health_log.txt", update_health=True)

def handle_general_log(chat_id, user_id, prompt):
    return handle_log(chat_id, user_id, prompt, "prompts/ask_farmer_log.txt")

def handle_local_practice_log(chat_id, user_id, prompt):
    return handle_log(chat_id, user_id, prompt, "prompts/ask_farmer_diy_log.txt")
...
def get_intent(prompt):
    system_instruction = load_prompt("prompts/ask_farmer_intent.txt")
    messages = [
